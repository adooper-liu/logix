...
// ✅ 符合单一职责
class ContainerFilterService { /* 只负责筛选 */ }
class SchedulingSorter { /* 只负责排序 */ }
class WarehouseSelectorService { /* 只负责仓库选择 */ }
class TruckingSelectorService { /* 只负责车队选择 */ }
class OccupancyCalculator { /* 只负责档期计算 */ }
class CostEstimationService { /* 只负责成本估算 */ }
```
