...
```vue
<template>
  <UniversalImport title="货柜数据导入" :field-mappings="CONTAINER_FIELD_MAPPINGS" api-endpoint="/api/import/excel/batch" />
</template>
...
<template>
  <!-- ❌ 独立的 UI 实现 -->
  <el-card class="demurrage-import-container"> ... </el-card>
</template>
```
...
```vue
<template>
  <UniversalImport title="滞港费标准导入" :field-mappings="DEMURRAGE_FIELD_MAPPINGS" api-endpoint="/api/import/demurrage-standards" :enable-batch-import="true" :batch-size="50" />
</template>
...
<template>
  <el-tabs v-model="activeTab">
    ...
    <el-tab-pane label="Excel 导入" name="excel">
      <UniversalImport title="飞驼数据导入" :field-mappings="FEITUO_FIELD_MAPPINGS" api-endpoint="/api/import/feituo" />
    </el-tab-pane>
  </el-tabs>
</template>
...
2. **简化组件代码**
   ...
   <!-- ExcelImport.vue -->
   <template>
     <UniversalImport title="货柜数据导入" :field-mappings="CONTAINER_FIELD_MAPPINGS" api-endpoint="/api/import/excel/batch" @success="handleSuccess" @error="handleError" />
   </template>
