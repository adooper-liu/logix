# ========================================
# 最晚提柜统计为 0 的诊断脚本 (PowerShell 版本)
# Diagnose why Last Pickup statistics show 0
# ========================================

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "最晚提柜统计诊断脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

$DB_CONTAINER = "logix-timescaledb-prod"
$DB_NAME = "logix_db"
$DB_USER = "postgres"

# 1. 检查 at_port 状态的货柜总数
Write-Host "1. 检查 at_port 状态的货柜总数..." -ForegroundColor Yellow
...
Write-Host "2. 检查有目的港操作记录的 at_port 货柜..." -ForegroundColor Yellow
docker exec -i $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c @"
...
# 3. 检查这些货柜是否有拖卡运输记录
Write-Host "3. 检查拖卡运输记录情况（关键查询）..." -ForegroundColor Yellow
docker exec -i $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c @"
SELECT 
    c.container_number,
    c.logistics_status,
...
# 4. 统计各种情况的数量
Write-Host "4. 统计分布情况（最重要）..." -ForegroundColor Yellow
docker exec -i $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c @"
SELECT 
...
Write-Host "5. 检查 lastFreeDate 字段填充情况..." -ForegroundColor Yellow
docker exec -i $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c @"
SELECT 
...
# 6. 查看所有物流状态分布
Write-Host "6. 查看所有物流状态分布..." -ForegroundColor Yellow
docker exec -i $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c @"
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "诊断完成！请查看以上结果分析原因。" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
