...
interface Props {
  /** 最晚还箱日期（数据库值） */
  lastReturnDate?: string | Date | null
  /** 最晚还箱日期（计算值，优先显示） */
  lastReturnDateComputed?: string | Date | null
  /** 计算模式（actual/forecast） */
  lastReturnDateMode?: 'actual' | 'forecast' | null
  /** 实际提柜日期（用于计算来源） */
  pickupDateActual?: string | Date | null
  /** 计划提柜日期（用于预测模式计算来源） */
  plannedPickupDate?: string | Date | null
  /** 是否显示完整信息（来源标注、计算模式） */
  showDetails?: boolean
  /** 是否显示状态标签（过期/紧急/正常） */
  showStatus?: boolean
...
const displayDate = computed(() => {
  return props.lastReturnDateComputed ?? props.lastReturnDate
})

// 是否为计算值
const isComputed = computed(() => !!props.lastReturnDateComputed)

// 计算模式文字
const modeText = computed(() => {
  if (!isComputed.value || !props.lastReturnDateMode) return ''
  return props.lastReturnDateMode === 'actual' ? '（实际模式）' : '（预测模式）'
})

// 计算来源文字
const sourceText = computed(() => {
  if (!isComputed.value) return ''
  if (props.lastReturnDateMode === 'actual') {
    return '起算：实际提柜日'
  } else {
    return '起算：计划提柜日（预测）'
  }
...
<template>
  <div class="last-return-date-display">
    <!-- 日期显示 -->
