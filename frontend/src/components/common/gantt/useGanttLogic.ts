...
export function useGanttLogic() {
  ...
  /** 拖拽起点所在节点行（清关/提柜/…），供 document 级 drop 与推断兜底，避免误用 infer 命中「清关」 */
  ...
  const pendingDropConfirm = ref<{
    ...
    /** 计划卸柜方式：仅 Drop off 走软约束；Live load / Expedited 等与提柜同步 */
    unloadMode?: string
    isDropOffMode?: boolean
    /** Drop off：卸柜前移且早于原提柜日，需在 dragend 询问是否同步提柜 */
  ...
  const getNodeAndSupplier = (container: Container): Array<{ node: string; supplier: string }> => {
    ...
    // 查验节点 - 清关行（与清关使用相同供应商）
    if (
      container.inspectionRequired &&
      container.portOperations &&
      container.portOperations.length > 0
    ) {
      const destPortOp = container.portOperations.find(op => op.portType === 'destination')
      const customsSupplier = destPortOp?.customsBrokerCode || destPortOp?.customsBroker
      if (customsSupplier) {
  ...
  const calculateDynamicDateRange = (containers: Container[]): [Date, Date] => {
    ...
    containers.forEach(container => {
      ...
      // 港口操作计划日期
      if (container.portOperations && container.portOperations.length > 0) {
        const portOp = container.portOperations[0]
        if (portOp.plannedCustomsDate) dates.push(new Date(portOp.plannedCustomsDate))
      }

      // 仓库操作计划日期
      if (container.warehouseOperations && container.warehouseOperations.length > 0) {
        const warehouseOp = container.warehouseOperations[0]
        if (warehouseOp.plannedUnloadDate) dates.push(new Date(warehouseOp.plannedUnloadDate))
      }

      // 还箱计划日期
      if (container.emptyReturns && container.emptyReturns.length > 0) {
        const emptyReturn = container.emptyReturns[0]
        if (emptyReturn.plannedReturnDate) dates.push(new Date(emptyReturn.plannedReturnDate))
      }
  ...
  const handleDrop = (date: Date, nodeName?: string) => {
    ...
    // 优先：模板传入的节点；其次：dragstart 记录的节点（document drop 未带参时依赖）；最后：分组推断（易误判为清关）
    ...
    // ✅ 关键修复：只要卸柜日被更新（无论是直接拖拽还是同步），都要重算还箱日
    if (updateData.plannedUnloadDate) {
  ...
  /**
   * ✅ 前向检查：目标日期必须 ≥ 前节点日期
   * @returns { passed: boolean, errorMessage?: string, sourceDate?: string }
   */
  const performForwardCheck = (
    ...
    switch (currentField) {
      case 'plannedCustomsDate':
        sourceDate = trucking?.plannedCustomsDate
          ? dayjs(trucking.plannedCustomsDate).format('YYYY-MM-DD')
          : null
        break
   ...
   * @returns { autoSyncFields: string[], needsUserConfirm: boolean }
   */
  ...
  const handleGlobalDrop = (event: DragEvent) => {
    ...
    let nodeFromTransfer: string | undefined
    try {
      nodeFromTransfer = event.dataTransfer?.getData('application/x-logix-node') || undefined
    } catch {
