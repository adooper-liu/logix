...
export function useFileUpload() {
  ...
  function groupByTable(transformed: Record<string, any>, fieldMappings: any[]) {
    ...
    const mainTableCN =
      tables.biz_containers?.container_number || tables.process_sea_freight?.containerNumber
    if (mainTableCN) {
      const subTables = [
      ...
      if (
        !tables.process_port_operations.container_number &&
        !tables.process_port_operations.containerNumber
      ) {
      ...
      if (transformed['目的港'] || transformed['port_of_discharge']) {
        portRecords.push({
          ...
          port_sequence: portRecords.length + 1,
          eta:
            transformed['预计到港日期(ETA)'] || transformed['eta'] || transformed['eta_dest_port'],
          ata:
            transformed['实际到港日期(ATA)'] || transformed['ata'] || transformed['ata_dest_port'],
  ...
  async function uploadBatchData(
    ...
    try {
      ...
      for (let i = 0; i < totalRecords; i += batchSize) {
        ...
        console.log(
          `[uploadBatchData] 发送批次 ${Math.floor(i / batchSize) + 1}, 数据量:`,
          batch.length
        )
        console.log(
          '[uploadBatchData] 第一条分组数据 JSON:',
          JSON.stringify(groupedBatch[0], null, 2)
        )
        ...
        try {
          ...
          console.log(
            '[uploadBatchData] result.success:',
            result.success,
            'type:',
            typeof result.success
          )
          console.log(
            '[uploadBatchData] result.failed:',
            result.failed,
            'type:',
          ...
          errors.push(
            `批次 ${Math.floor(i / batchSize) + 1} 上传失败：${batchError instanceof Error ? batchError.message : '未知错误'}`
          )
        }
