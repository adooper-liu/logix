...
import type { ColumnKey, ColumnVisibleConfig, ContainerTableProps } from './types'
import { DEFAULT_COLUMN_ORDER, DEFAULT_COLUMN_VISIBLE } from './types'
 ...
 * @returns 表格逻辑相关状态和方法
 */
export function useContainerTable(props: ContainerTableProps): UseContainerTableReturn {
  // 列显示配置
  ...
  // 列顺序
  const columnOrder = ref<ColumnKey[]>([...(props.columnOrder || DEFAULT_COLUMN_ORDER)])
  ...
  function saveToStorage() {
    try {
      localStorage.setItem(STORAGE_KEY_VISIBLE, JSON.stringify(columnVisible.value))
