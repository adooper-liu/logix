...
```typescript
// 拖拽确认后，调用单柜优化
async function handleDragConfirm(container: Container, newDate: string, updateField: string) {
  // 1. 先更新日期
  ...
  if (updateField === 'plannedPickupDate' || updateField === 'plannedUnloadDate') {
    const optimizationResult = await costOptimizationService.optimizeContainer({
      ...
      basePickupDate:
        updateField === 'plannedPickupDate' ? newDate : calculateBasePickupDate(newDate),
    })
