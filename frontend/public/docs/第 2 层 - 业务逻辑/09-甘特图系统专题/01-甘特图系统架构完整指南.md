...
甘特图 Tooltip 中显示基于映射关系的候选供应商（车队和仓库），帮助用户快速了解可用的资源选项。

### 6.2 映射关系链

```
目的港 (portCode) → dict_trucking_port_mapping → 车队
车队 (truckingCompanyId) → dict_warehouse_trucking_mapping → 仓库
```
...
<div v-if="tooltipContainer?.availableTruckingCompanies?.length || tooltipContainer?.availableWarehouses?.length" class="tooltip-section candidate-section">
  ...
  <div v-if="tooltipContainer?.availableTruckingCompanies?.length" class="candidate-group">
    ...
    <div class="candidate-items">
      <span
        v-for="truck in tooltipContainer.availableTruckingCompanies"
        :key="truck.truckingCompanyId"
        class="candidate-tag"
        :class="{ 'is-default': truck.isDefault, 'is-current': ... }"
      >
...
## 七、静态映射与三级供应商结构（v1.3 新增）

### 7.1 功能概述

甘特图使用静态映射表构建三级供应商结构（港口→车队→仓库），确保供应商显示不依赖货柜数据，同时保持数据一致性。

### 7.2 映射关系

```
dict_trucking_port_mapping          dict_warehouse_trucking_mapping
┌─────────────────────────────┐     ┌─────────────────────────────┐
│ port_code    VARCHAR(10)    │     │ trucking_company_id VARCHAR │
│ port_name    VARCHAR(100)   │     │ trucking_company_name      │
│ country      VARCHAR(10)     │────▶│ warehouse_code    VARCHAR  │
│ trucking_company_id         │     │ warehouse_name    VARCHAR  │
│ trucking_company_name        │     │ country           VARCHAR │
...
**接口**: `GET /warehouse-trucking-mapping/static`
...
**核心原则**:
1. **只显示有货柜的港口** - 通过 `finalFilteredContainers` 收集有货柜的目的港
2. **供应商结构来自映射表** - 使用静态映射构建五节点下的供应商列表
3. **货柜叠加到节点** - `getNodeAndSupplier()` 将货柜分配到对应节点
...
const staticBasedGroupedByPort = computed(() => {
  ...
  staticMappings.value?.ports?.forEach(portInfo => {
    const { portCode, portName, portCountry } = portInfo

    // 只显示有货柜的港口
    if (!portsWithContainers.has(portCode)) return
    ...
    // 提柜节点：显示车队（来自 dict_trucking_port_mapping）
    truckingList.forEach(t => {
      groups[portCode]['提柜'][t.truckingCompanyId] = []
    })

    // 卸柜节点：显示仓库（来自 dict_warehouse_trucking_mapping）
    ...
    truckingList.forEach(t => {
      ...
      warehouses.forEach(w => warehouseKeys.add(w.warehouseCode))
    })
  ...
  finalFilteredContainers.value.forEach(container => {
    const portCode = container.destinationPort || '未指定'
    if (!groups[portCode]) groups[portCode] = { /* 初始化 */ }
...
├── staticMappings: ref<StaticMappings>     // 静态映射数据
├── finalFilteredContainers: computed      // 过滤后的货柜
├── staticBasedGroupedByPort: computed      // 三级分组（核心）
│   ├── 步骤1：收集有货柜的港口
│   ├── 步骤2：用静态映射构建供应商结构
│   └── 步骤3：叠加货柜到节点
└── loadStaticMappings()                    // 加载静态映射
```

### 7.7 国别筛选支持

静态映射 API 支持全局国别筛选：
- 通过 `X-Country-Code` header 传递国别
- 后端 `getScopedCountryCode()` 获取筛选值
- 只返回匹配国别的港口-车队-仓库映射
...
- 新增 七、静态映射与三级供应商结构章节
- 说明静态映射 API `/warehouse-trucking-mapping/static`
- 说明港口-车队-仓库显示规则
- 说明三级分组构建逻辑（只显示有货柜的港口）

### v1.2 (2026-04-01)

- 新增 六、候选供应商显示章节
- Tooltip 新增可用供应商区（基于映射关系）
- 后端 enrichContainersList 增加 availableTruckingCompanies/availableWarehouses 字段
- 候选供应商按 isDefault 排序，默认选项显示 ⭐ 标记
