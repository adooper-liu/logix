...
- **含义**：按当前泳道 + 该行标签，用与 Shipments 统计卡片一致的规则筛出属于该组的货柜；同时为每个货柜算出一个 **extractedDate**（该泳道下的「关键日期」）：
  - **按到港**：按5级优先级取日期（与 `extractDateFromContainer` 逻辑一致）：
    1. `container.ataDestPort`（实际到港日）
    2. `container.etaCorrection`（修正ETA）
    3. `container.etaDestPort`（预计到港日）
    4. `container.portOperations`中destination类型的港口操作记录的`ataDestPort/etaCorrection/etaDestPort`
    5. `container.seaFreight.eta`（海运表的目的港ETA）← 最后fallback，用于没有港口操作记录的货柜
...
| **composables/useGanttHelpers.ts** | getRemainingTime、formatDateLabel、formatFullDate、extractDateFromContainer（5级优先级取日期：ataDestPort > etaCorrection > etaDestPort > portOperations[destination] > seaFreight.eta）。 |
| **types/ganttChart.ts** | LaneConfig、ContainerItem、TimeGroup、RemainingTime 等类型。 |
| **backend/src/services/container.service.ts** | enrichContainersList方法（第106行）：实现5级优先级日期回写，`etaDestPort: latestPortOperation?.etaDestPort || container.seaFreight?.eta || null`。 |
...
| 为什么有的货柜没有港口操作记录也能显示？ | 后端 enrichContainersList 在 latestPortOperation 为空时会使用 seaFreight.eta 作为 etaDestPort 的 fallback，确保只有海运记录的货柜也能正常显示。 |

---

*与《甘特图四泳道与 Shipments 统计卡片子集逻辑对比》配合使用：分组子集逻辑见对比文档，本页侧重「如何画出来」的显示流程。*

*供应商分组与五阶段流转详见 [11-甘特图三级供应商展开逻辑](./11-甘特图三级供应商展开逻辑.md)。*
