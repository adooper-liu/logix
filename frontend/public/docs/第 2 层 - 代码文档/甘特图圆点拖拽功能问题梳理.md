...
const handleDrop = (date: Date) => {
  ...
  const matchedItem = displayItems.find(
    item => item.plannedDate && dayjs(item.plannedDate).format('YYYY-MM-DD') === targetDate
  )
...
const retryLastDrag = async () => {
  ...
  try {
    const result = await containerService.updateSchedule(lastDragData.container.containerNumber, {
...
describe('Drag and Drop Integration', () => {
  test('should complete drag-drop flow successfully', async () => {
    ...
    // Verify API called
    await waitFor(() => {
      expect(containerService.updateSchedule).toHaveBeenCalledWith('HMMU6232153', {
