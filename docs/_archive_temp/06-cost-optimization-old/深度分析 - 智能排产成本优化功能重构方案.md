...
<template>
  <el-dialog v-model="showDialog" title="💰 成本优化分析报告">
    <OptimizationResultCard :report="currentReport" @accept="handleAccept" @reject="handleReject" />
  </el-dialog>
</template>
```
...
```vue
<!-- 当前代码 -->
<OptimizationAlternatives :alternatives="currentAlternatives" @select="handleAlternativeSelect" />

<!-- 改为 -->
<OptimizationResultCard :report="bulkOptimizationReport" @accept="handleAccept" @reject="handleReject" />
```
