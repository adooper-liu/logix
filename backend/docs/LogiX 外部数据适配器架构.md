...
LogiX 外部数据适配器架构提供了一个统一的接口来处理多个外部数据源（如飞驼 API、物流路径微服务等），实现了：

> **飞驼 API 参考**：[飞驼可视接口](https://doc.freightower.com/) 提供集装箱综合跟踪（28 节点）、异常预警推送、智能 ETA 预测、船期计划等；当前 FeiTuoAdapter 主要对接**集装箱综合跟踪**的订阅与查询。
