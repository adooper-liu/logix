...
-- ============================================================
-- 注意：如果此表不存在，以下语句会失败，请先创建表或跳过此部分

-- 3.1 添加 slot_type 字段（仅当表存在时执行）
-- 先检查表是否存在
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ext_trucking_slot_occupancy') THEN
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_name = 'ext_trucking_slot_occupancy' AND column_name = 'slot_type'
    ) THEN
        EXECUTE 'COMMENT ON COLUMN ext_trucking_slot_occupancy.slot_type IS ''档期类型：delivery=送柜，return=还箱''';
    END IF;
END $$;
