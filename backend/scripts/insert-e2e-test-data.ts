...
async function insertTestData() {
  ...
  try {
    ...
    await queryRunner.query('DELETE FROM ext_container_status_events WHERE container_number = $1', [testContainerNumber])
    await queryRunner.query('DELETE FROM process_empty_return WHERE container_number = $1', [testContainerNumber])
    await queryRunner.query('DELETE FROM process_warehouse_operations WHERE container_number = $1', [testContainerNumber])
    await queryRunner.query('DELETE FROM process_trucking_transport WHERE container_number = $1', [testContainerNumber])
    await queryRunner.query('DELETE FROM process_port_operations WHERE container_number = $1', [testContainerNumber])
    await queryRunner.query('DELETE FROM process_sea_freight WHERE bill_of_lading_number = $1', [`BL_${testContainerNumber}`])
    await queryRunner.query('DELETE FROM biz_containers WHERE container_number = $1', [testContainerNumber])
    ...
    await queryRunner.query(
      ...
      [testContainerNumber, truckingCompanyId, plannedPickupDate, plannedUnloadDate, 'Direct', now, now]
    )
