...
async function testExcelImport() {
  ...
  try {
    ...
    // 测试 1: 验证飞驼 Excel 导入服务
    console.log('\n1. 测试飞驼 Excel 导入服务');
    try {
      // 测试导入方法存在性
      if (typeof feituoImportService.import === 'function') {
        console.log('✅ 飞驼导入服务方法存在');
      } else {
        console.log('❌ 飞驼导入服务方法不存在');
      }

      // 测试表类型检测
      const testColumns1 = ['MBL Number', '集装箱号'];
      const testColumns2 = ['提单号', '港口代码'];
      
      console.log('✅ 飞驼导入服务初始化成功');
    } catch (error) {
      console.log('❌ 飞驼导入服务测试失败:', error);
    }

    // 测试 2: 验证普通 Excel 导入控制器
    console.log('\n2. 测试普通 Excel 导入控制器');
    try {
