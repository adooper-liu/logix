...
async function testLogisticsPath() {
  try {
    ...
    const response = await fetch(
      'http://localhost:4000/api/v1/logistics-path/container/HMMU6232153'
    );
    ...
    if (data.data && data.data.nodes) {
      ...
      // 查找提柜阶段的节点
      const pickupNodes = data.data.nodes.filter(
        (n: any) =>
      ...
      data.data.nodes.forEach((n: any, i: number) => {
        console.log(
          `${i + 1}. ${n.status} - ${n.statusCode || 'N/A'} - ${n.description || 'N/A'} - ${n.rawData?.dataSource || 'N/A'}`
        );
      });
    } else {
      console.log('没有返回数据');
