...
function withBatchSize(size: string, run: () => Promise<void>): Promise<void> {
  ...
  return run().finally(() => {
    if (original === undefined) {
      delete process.env.DEMURRAGE_BATCH_SIZE;
    } else {
      process.env.DEMURRAGE_BATCH_SIZE = original;
    }
  });
}
...
describe('DemurrageWriteBackScheduler rotating compute batch', () => {
  it('advances offset when a full page is processed', async () => {
    await withBatchSize('2', async () => {
      const batchComputeAndSaveRecords = jest
        .fn()
      ...
      const scheduler = createScheduler(batchComputeAndSaveRecords);

      await scheduler.triggerManualComputeBatch();
      expect(batchComputeAndSaveRecords).toHaveBeenLastCalledWith(
  ...
  it('retries from offset 0 when current offset is past the end', async () => {
    await withBatchSize('2', async () => {
      ...
      await scheduler.triggerManualComputeBatch();
      expect(batchComputeAndSaveRecords.mock.calls.slice(-2)).toEqual([
        [expect.objectContaining({ limit: 2, offset: 2 })],
        [expect.objectContaining({ limit: 2, offset: 0 })]
      ]);
      expect(scheduler.getNextOffset()).toBe(2);
