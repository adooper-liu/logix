...
router.get('/memory-analysis', async (_req: Request, res: Response) => {
  try {
    ...
    const memoryUsage = Math.round(
      (processMemoryUsage.heapUsed / processMemoryUsage.heapTotal) * 100
    );
...
async function getOptimizationMetrics() {
  try {
    ...
    // 执行查询以确保连接正常（结果暂时不使用）
    void Promise.all([containerRepo.count(), orderRepo.count()]);
 ...
 * 生成告警信息
 */
function generateAlerts(performanceMetrics: any, serviceHealth: any) {
  const alerts: any[] = [];
