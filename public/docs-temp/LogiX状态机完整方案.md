...
#### TruckingTransportService集成

```typescript
@Injectable()
export class TruckingTransportService {
  constructor(
    private containerStatusService: ContainerStatusService
  ) {}

  async createTruckingTransport(data: CreateTruckingTransportDto): Promise<TruckingTransport> {
    const trucking = await this.truckingTransportRepository.save(data);
...
export class WarehouseOperationService {
  ...
  private isWmsConfirmed(data: CreateWarehouseOperationDto): boolean {
    return data.wmsStatus === 'WMS已完成' ||
           data.ebsStatus === '已入库' ||
           data.wmsConfirmDate !== null;
  }
}
```

#### EmptyReturnService集成

```typescript
@Injectable()
export class EmptyReturnService {
  constructor(
    private containerStatusService: ContainerStatusService
  ) {}

  async createEmptyReturn(data: CreateEmptyReturnDto): Promise<EmptyReturn> {
    const emptyReturn = await this.emptyReturnRepository.save(data);
...
```typescript
async checkDataIntegrity(): Promise<{ inconsistent: string[]; missing: string[] }> {
  const inconsistent = await this.getInconsistentContainers();
  const missing = await this.getMissingDataContainers();
...
```typescript
// 统计逻辑
async getLastPickupStatistics() {
  const result = await this.containerRepository.query(`
    SELECT
      CASE
        WHEN po.last_free_date IS NULL THEN '缺最后提柜日'
        WHEN DATE(po.last_free_date) < CURRENT_DATE THEN '已逾期未提柜'
        WHEN DATE(po.last_free_date) <= CURRENT_DATE + INTERVAL '3 days' THEN '紧急：倒计时3天内'
        WHEN DATE(po.last_free_date) <= CURRENT_DATE + INTERVAL '7 days' THEN '警告：倒计时7天内'
        ELSE '正常'
      END as category,
      COUNT(*) as count
    FROM biz_containers c
...
// 前端显示逻辑
function renderStatusBadge(statusInfo: ContainerStatusInfo) {
  if (!statusInfo.isConsistent) {
    return (
      <Badge type="warning">
        {statusInfo.currentStatus}
        <Tooltip text={statusInfo.warnings.join(', ')} />
      </Badge>
    );
  }

  return <Badge type="normal">{statusInfo.currentStatus}</Badge>;
}
```
